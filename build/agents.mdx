...
| **Voice** | TTS voice selection, STT configuration, interruptions, turn context, background sound |
...
The Voice Gateway detects automatically when the caller has finished speaking. There is no manual tuning: silence thresholds and voice activity detection are handled by the gateway's turn-taking engine.

<Note>
  Earlier versions exposed **Silence Confirmation Delay** and **Semantic VAD** controls. These settings are now managed by the Voice Gateway and no longer appear in the Studio UI. Existing agents keep their stored values for backward compatibility.
</Note>

### Turn context

Free-text context that helps the gateway decide when the caller has finished their turn. Use it to describe expected answers, pauses, spelling, or number sequences.

For example, if callers spell out a reference number character by character, mention it here so the agent waits for the full reference instead of responding after the first pause.

Open it from the **Turn context** card in the Voice tab.

### Allow interruptions

Controls whether the caller can interrupt the agent while it's speaking.

| Setting | Description | Default |
|---------|-------------|---------|
| **Allow interruptions** | Let the caller interrupt the agent while it is speaking | Off |
