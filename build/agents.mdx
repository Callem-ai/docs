...
The silence duration (in milliseconds) before the system considers the caller has finished speaking. Lower values make the agent more responsive but may cut the caller off. Higher values are more patient but introduce latency.

| Value | Behavior |
|-------|----------|
| **200-400ms** | Very responsive, risks interrupting pauses |
| **500-800ms** | Good balance for most use cases |
| **1000ms+** | Very patient, best for elderly callers or complex topics |
...
| **Minimum words** | Minimum words the caller must say before the interruption is registered | `3` |
| **Minimum duration** | Minimum duration (ms) of caller speech before interruption triggers | `15` |

### Reminder Message

Time in seconds before the agent sends a reminder message if the caller remains silent. Set to `0` to disable.

### Voice Speed

Adjust the speaking speed of the agent's voice. `1.0` is normal speed.
...
Call Analysis is a **post-call processing** feature that uses an LLM to extract structured data from call transcripts after the call ends. This is useful for automatically categorizing calls, extracting key information, and feeding data into your analytics dashboards.
