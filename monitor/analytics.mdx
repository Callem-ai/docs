...
| **Channel** | Multi-select: Voice, Chat (or both). Future channels will appear here. |
...
<AccordionGroup>
  ...
  <Accordion title="Use the channel filter for channel-specific views">
    To compare or isolate performance by channel, use the **Channel** filter: select only **Voice** for phone metrics, only **Chat** for web widget metrics, or both for combined conversation metrics. Custom dashboard charts respect the same channel selection.
  </Accordion>
</AccordionGroup>
