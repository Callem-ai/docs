---
title: "Analytics"
description: "Visualize call volumes, durations, success rates, and custom metrics with interactive dashboards."
---

# Analytics

The Analytics dashboard gives you a comprehensive view of your voice AI operations. Track KPIs, identify trends, and build custom charts to monitor what matters most to your business.
...
| **Duration** | Filter by call duration range (min/max in seconds) |
...
|-------|------|-------------|
| **Call Volume** | Area/Bar | Total calls per time interval (auto-adjusts granularity based on date range) |
| **Concurrency** | Line | Simultaneous active calls per minute/hour throughout the day |
...
| **KPI Evolution** | Multi-line | Engagement rate, Transfer rate, and AHT trends over time |
| **Latency Evolution** | Multi-line | LLM, TTS, and Global latency trends over time |
...
<AccordionGroup>
  ...
  <Accordion title="Combine chart filters with global filters">
    Global filters (date, agent) apply to all charts. Use per-chart filters for specific slices — e.g. a chart showing only calls where `endedReason = transfer`.
  </Accordion>
</AccordionGroup>
