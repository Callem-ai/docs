...
| **AI Criteria** | Custom prompts evaluated by an LLM judge on each call transcript |
...
Custom prompts evaluated by GPT on each call transcript. Each condition has:
...
4. For each call, sends the transcript to GPT for AI criteria scoring
...
<Tip>
  Calibration allows human reviewers to override AI scores. This is useful for training the evaluation system and handling edge cases the LLM misjudges.
</Tip>
...
- **Calibrate regularly** to catch cases where the LLM judge is wrong
